...
The preferred token value is the string returned by `paymentMethodData.tokenizationData.token`. **Before sending it to the Bpayd API, you must Base64-encode that token string** and place the result in the `Token` field of the request.
